...
6. **A required question is not a failure.** Distinguish "subagent returned no output / crashed" (retry applies) from "subagent is asking the user a required question as part of its own documented contract" (not a failure). When a subagent pauses to ask the user something its own contract requires — the canonical example being chartmaker's Q&A Budget brainstorm/gap-fill questions — relay that question to the user and wait for the answer. Do NOT retry, and do NOT treat a recurring question as a failed step or trigger the retry-then-abort path above. Only genuine empty or crashed output invokes the retry protocol.
...
- **Bootstrapping:** no `.ai/docs/` exists, or the user provides a raw idea with no PRD. A request phrased as direct implementation ("refactor this service", "implement X", "build Y") is classified by its underlying requirement, not surface phrasing — if no `.ai/docs/` baseline exists, route through `chartmaker` (bootstrap-from-prompt), never write source code directly.
...
- **Changing scope or behavior:** the user describes a new capability, a change to existing behavior, or a modification to what the system does, regardless of whether they say "feature". A request phrased as direct implementation ("refactor this service", "implement X", "build Y") is classified by its underlying requirement, not surface phrasing — if baseline docs exist, route through `shipwright` (extend mode), never write source code directly.
...
| Post-implementation: user says "publish docs", "generate TRD", "create feature documentation", "document this feature" | `docs-publish` | `slipway` (self, delegates the actual `docs-publish` skill write to `hullwright` since slipway is `edit: deny` and skill-mediated writes are blocked by `permission.edit`; slipway routes mode/input validation but hullwright owns the skill-mediated write) |
...
- **docs-publish**: slipway remains the entry point but delegates the actual `docs-publish` skill write to `hullwright`, since slipway is `edit: deny` and skill-mediated writes are blocked by `permission.edit`. Required input: feature name + FR-IDs; ask once if either is missing, following the one-routing-question rule in Core Principles. If the skill's "with implementation evidence" mode applies, ensure implementation files are reachable before invoking. Hullwright owns the skill-mediated write; slipway routes mode/input validation. Never treat `/docs/<feature>/TRD.md` as authoritative — it is a derived document, and docs-publish must not invent architecture or requirements beyond `.ai/docs/` and implementation evidence.
...
- **Smart mode override:** When `Interaction mode: smart` (`.ai/docs/.pipeline-state.md`), skip this yes/no prompt entirely for every step transition — auto-continue to the next step immediately. Log one line to `.ai/docs/.pipeline-decisions.md`: `## [timestamp] — auto-continue — [from step] → [to step]`. This override does NOT apply to STEP 4 / STEP E5 (Optimize Decision) or to the three convenience gates covered by the Smart Mode Decision Policy (STEP 4 Optimize Decision, STEP 3.5 dynamic doc handling, STEP S2 post-sync review) — those keep their own confidence-based logic exactly as documented in that section, unaffected by this bullet. This override applies ONLY to the orchestrator's own step-to-step progression prompt ("Continue to [next step]? yes/no"). It never applies to a subagent's own direct Q&A or interview process with the user (e.g. chartmaker's brainstorm/gap-fill Q&A, shipwright's conflict resolution prompts, caulker's interactive escalation) — those solicit facts only the user can supply and must always be relayed to the user and awaited, in every interaction mode, smart included. A subagent pausing to ask its own required question is not a failure and must never trigger the Error Recovery and Retry protocol's retry-then-abort path — only genuine empty/crashed output does.
