...
- Generate all engineering documents in order
- Compile `AGENT.md`
...
- Delete all generated docs (`02` through `10-planning-rules.md` and `AGENT.md`)
- Preserve only `.ai/docs/01-prd.md` and any supplementary PRDs (files prefixed `11-` or higher that end in `-prd.md`)
- Regenerate all engineering documents from scratch
...
Generate in this exact order. For each document, use the corresponding template in `templates/` and populate it using **only** the Requirement Model built in STEP 2.
...
- Add every generated dynamic doc to `10-planning-rules.md`'s source-of-truth list and to `AGENT.md`'s Source Of Truth section. The `inspector` subagent validates this list — if it's hardcoded to `02–10`, it will miss dynamic docs.
