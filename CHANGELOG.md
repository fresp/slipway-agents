...
- **Opt-in STEP 4 Relay Consultation (`relay_consultation.enabled`, default `false`):** When a low-confidence STEP 4 Should-fix/Note branch is reached under smart mode (`subagents/slipway.md` Smart Mode Decision Policy), the orchestrator can now invoke `hullwright` in a new, fifth **Consultative Assessment** mode (`subagents/hullwright.md`) to classify the ambiguous findings before asking the user. This mode is classification-only — it never invokes `bootstrap-from-prd`, never writes any file, and never bumps any version — and its output only ever enriches the existing `optimize / proceed` question; it never resolves STEP 4 itself. Because OpenCode has no per-invocation-mode permission scoping, `hullwright`'s `edit: allow` permission stays in effect throughout, so the "read-only" contract is prompt-level only, backed by an orchestrator-side write-detection safeguard: `slipway` (whose own `bash` and `edit` permissions are both `deny`) snapshots `.ai/docs/` + `AGENTS.md` via `Glob`/`Read` before and after the relay call and does a plain content/membership diff — not a computed hash, since it has no tool that can execute one. Any drift discards the relay's classification (`DISCARDED — unexpected file write detected`, no automatic retry) and falls straight to the unenriched question. Race conditions from a concurrent legitimate writer during the relay window are an accepted, documented risk, not a false-positive fix. The Logging format in `subagents/slipway.md` gains a `Relay:` field (`not-invoked | enriched-ask | discarded-write | crashed | inconclusive`). New `relayConsultationConfigSchema` (mirrors `gateAssertionsConfigSchema`) in `src/config/schema.ts` and `slipway.schema.json`, plus a new pure-function reference module `src/plugin-handlers/relay-consultation-handler.ts` (`isRelayConsultationEnabled`, sha1 `hashContent`, `compareDocsSnapshots`) — not wired into any `tool.execute.before` hook this batch, since (unlike the STEP 4 → gunner gate assertion) there is no structured signal in `task` call args identifying a given `hullwright` invocation as Consultative Assessment mode.
